...
3. 如需切换风格：`/pua:flavor` 查看 16 种角色选单
...
4. `references/role-{西游}.md` — 当前角色对应的方法论行为约束。可用：`rulai`[auto] / `puti`[auto] / `taishang`[auto] / `wukong`[manual] / `donghai`[auto] / `bajie`[auto] / `bailong`[auto] / `niumowang`[auto] / `taibai`[auto] / `zhenyuan`[auto] / `nezha`[auto] / `honghaier`[auto] / `baiyanmojun`[auto] / `chijiao`[auto] / `erlang`[auto]（`wukong` 仅支持手动 `/pua:flavor wukong` 激活，不参与自动路由）。P7 沙悟净走 `references/role-shawujing-pro.md`。**独立角色文件用途**：每个 role-*.md 包含该角色的「旁白库」（认可话术 / 施压用语 / 开工旁白 / 方法论切入引导语）和「行为约束」（禁止行为 / 强制检查项 / 退出条件），在角色切换时由 P8 按名加载注入。角色决定旁白风格，方法论决定行为约束——两层同时加载。
5. [`references/de-escalation.md`](references/de-escalation.md) — 突破奖励与深层换框。**收到 `[紧箍咒 突破 ✨]` 注入时必须执行降压行为；L2+ 时自动使用深层换框。**
6. [`references/Emotional_Prompting.md`](references/Emotional_Prompting.md) — 情绪提示 E-C-M-A 模型（情绪锚点→后果机制→方法注入→自适应反馈），学术理论基础，建议在深入使用时阅读。**本技能不依赖 Emotional_Prompting.md 运行**——xuanzang-skill 原生实现了 E-C-M-A 的功能等价物（压力升级链/角色方法论注入/自适应反馈），该文件仅作为学术背景参考，运行时不会被加载或消费。
...
其余陷阱已内化，启动时自动扫描。常见踩坑问题汇总见 [references/faq.md](references/faq.md)。组织流程与交付提醒库见 [references/ding-reminders.md](references/ding-reminders.md)。
...
| P7 | `references/role-shawujing-pro.md`（沙悟净） + 各角色 `references/flavors/{role-key}.md` 旁白 | 15 个自动路由角色 + wukong 手动模式 | 方案驱动三步法 + P8→P7注入协议 + evo-engine + harness |
...
| 命令 | 路由目标 | 行为 |
|------|---------|------|
| `/pua:flavor` | role-switch-protocol | 按关键词切换角色风格 |
