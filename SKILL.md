...
**失败计数持久化**：失败次数在 context compaction 时由 PreCompact hook 自动保存到 `~/.xuanzang/builder-journal.md`，SessionStart hook 自动恢复。详见 下方 ## Evolution Protocol：自进化协议。
...
- 核心行为：直接 Read 文件 SKILL.md 中 ## Harness Governance：防作弊治理协议 节的防作弊治理引擎协议
- 面板格式：同目录下的 下方 ## Display Protocol
- 如果是 P7 模式：同目录下的 references/role-juanlian-pro.md
...
| 🔴 菩提祖师 | > [🔴 菩提祖师味] 灵台方寸之间，自有大千。破执——你不悟，我点。炮火准备好了吗？ | 灵台方寸 · 渡劫 |
...
- 突破发生时 failure-detector.py 自动追加方法论沉淀骨架到 `~/.xuanzang/下方 ## Evolution Protocol 的 evolution.md`（包含连续失败次数、模式、空位字段）
...
`~/.xuanzang/下方 ## Evolution Protocol 的 evolution.md` 是自进化的持久化存储。不存在时自动创建。
...
检查 `~/.xuanzang/下方 ## Evolution Protocol 的 evolution.md` 是否存在：
