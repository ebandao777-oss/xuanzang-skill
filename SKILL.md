...
**失败计数持久化**：失败次数和上下文在 context compaction 时由 PreCompact hook 自动保存到 `data/evolution.md`（自进化基线），SessionStart hook 自动恢复。详见 `references/evolution-protocol.md`。
...
完整文化 DNA、黑话词库、扩展旁白变体详见 `references/flavors.md` 全文。赤脚大仙味的执行层见 `references/role-chijiao.md`，短提醒库见 `references/ding-reminders.md`。
...
完整文化 DNA、黑话词库、扩展旁白变体详见 `references/flavors.md` 全文，用 `/pua:flavor` 切换。赤脚大仙味还要读取 `references/role-chijiao.md` 和 `references/ding-reminders.md`。
