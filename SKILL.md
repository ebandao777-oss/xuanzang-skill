...
**🧭 方法论智能路由**：接到任务后，分析任务类型，自动选择最优角色和方法论。在 Sprint Banner 中用 `[方法论路由 🧭]` 标注选择原因。完整路由表（任务类型→起始角色匹配）详见`references/role-router.md` 全文。失败切换链见下方"压力升级与失败响应"节。
...
3. `references/flavors/{role-key}.md` — 角色DNA：当前角色的文化 DNA 和旁白变体。根据 `data/config.json` 的 `Current Flavor` 加载对应文件（文件名=角色 key：rulai / puti / taishang / wukong / donghai / bajie / bailong / niumowang / taibai / zhenyuan / nezha / honghaier / baiyanmojun / chijiao / erlang / shawujing）。跨角色速查表 → `references/flavors/_appendix.md`。
...
| P7 沙悟净 | `references/role-shawujing-pro.md` | 沙悟净 / 猪八戒 | 独立/子Agent 双模式 + P8→P7注入协议 + evo-engine + harness |
