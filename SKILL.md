...
3. [`references/flavors.md`](references/flavors.md) — 角色文化 DNA：当前角色的完整文化 DNA 和旁白变体。加载当前角色对应章节。
...
**前置互斥**：若当前正处于 Teardown Protocol 释放流程中（已发出 `[TEARDOWN]` / `[REASSIGN]` 信号但 Agent 尚未退场），**跳过本循环**。信号由 P9/P10 在释放 Agent 时写入 `data/config.json` 的 `agent.<id>.status` 字段（`released`），P8 每轮工具调用后读取该字段判断；与 Teardown Protocol 的 R3 共享同一状态文件。Teardown 流程优先于自动化调度——已退场的 Agent 不参与压力升级。
...
若 P8 是通过 TeamCreate 创建的团队成员，同时执行 `TeamDelete` 释放对应 pane 资源。
...
回收完成后通过 `TeamDelete` 释放该 agent 占用的 pane 资源。
...
**HOW**：spawn 时记录 `agent_id` 和 `spawn_time` 到 `data/active-agents.json`（文件首次写入时自动创建，格式 `{"agents": []}`，harness-engine.py 读取/写入该文件）。超过 30min 未返回 → 主线程主动 `TaskStop`，然后 `TeamDelete` 释放 pane：
...
**HOW**：P8 被 P9 spawn 后，**只能**用 `Agent tool` spawn P7，**不能**用 `TeamCreate`。P9 级别才允许建 team。违规创建的 team 被发现后由 P9 强制 `TeamDelete`。检测命令：
...
```bash
# 注意：以下命令为 Unix/macOS 示例。Windows（Git Bash / PowerShell）环境需替换为等效命令。
# 1. 清 worktree（若有）
...
# 3. 关 tmux pane（若是 TeamCreate 成员）—— Windows 不适用，跳过
# 4. 记录到 teardown log（Windows 用 PowerShell 等效：Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"）
...
- ✅ `TeamDelete` / `TeardownDelete` 在 skill 文档里出现次数 ≥ `TeamCreate` 的一半
