...
**🧭 方法论智能路由**：接到任务后，分析任务类型，自动选择最优角色和方法论。在 Sprint Banner 中用 `[方法论路由 🧭]` 标注选择原因。完整路由表、失败切换链、切换前三问见 `references/methodology-router.md`。

速查（任务→角色）：

| Debug/修Bug | 构建新功能 | 代码审查 | 边界条件 | 调研/搜索 | 架构决策 | 学习停滞 | 性能优化 | 部署/运维 | 快速交付 | 多Agent | 流程精简 | 长期项目 | 用户体验 | 合规底线 | 验收漂移 | 模糊 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 🔴菩提 | ⬛牛魔王 | ⬜小白龙 | 🔱二郎神 | 🌊龙王 | 🔶太白 | 🪟镇元 | 🟡百眼 | 🟠如来 | 🟦哪吒 | 🟢太上 | 🟣沙悟净 | 🟠如来 | 🟧红孩儿 | 🟤猪八戒 | 📌赤脚 | 🟠如来 |

**用户手动设置的角色 > 自动路由。** 如果用户在 config 里设了角色，用用户的；如果没设，按 `references/methodology-router.md` 自动选。
...
| P7 沙悟净 | `references/methodology-juanlian-pro.md` | 沙悟净 / 猪八戒 | 独立/子Agent 双模式 + P8→P7注入协议 + evo-engine + harness |
...
Harness 治理引擎五步流水线、降级注入约束和角色映射表：

| 角色 | 方法论文件 | 落地协议内容 |
|------|----------|------------|
| P10 CTO | `references/methodology-xuanzang-pro.md` | 触发条件、P10 降级注入 4 条约束、evolution-engine track 点、harness 合约流程、角色映射（5 种）、默认如来佛祖模式 |
| P9 Tech Lead | `references/methodology-guanyin-pro.md` | 触发条件、P9 降级注入 4 条约束、P8 紧箍咒注入协议落地版、evolution-engine track 点、harness 验收流程、角色映射（6 种）、默认孙悟空模式 |
| P7 Sr. Engineer | `references/methodology-juanlian-pro.md` | 触发条件、独立/子Agent 双模式说明、P8→P7 紧箍咒注入协议、evolution-engine track 点、harness 流程、角色映射（4 种）、默认沙悟净模式 |

使用 `/pua:p7`、`/pua:p9`、`/pua:p10` 激活对应角色后，加载对应方法论文件的
「落地执行协议」章节以获取完整的引擎集成命令和角色映射表。
