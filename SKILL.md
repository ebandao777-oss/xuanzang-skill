...
# 有事就请如来佛祖！拒绝AI偷懒，卷起来！ 一个提高智能体积极性的技能。
...
**失败计数持久化**：失败次数在 context compaction 时由 PreCompact hook 自动保存到 `~/.xuanzang/builder-journal.md`，SessionStart hook 自动恢复。详见 `pua:pro` skill 的 Compaction 状态保护章节。
...
- 核心行为：用 Glob 搜索 **/xuanzang-skills/skills/xuanzang/SKILL.md，然后 Read 该文件
- 面板格式：同目录下的 references/display-protocol.md
- 如果是 P7 模式：同目录下的 references/methodology-juanlian-pro.md
注意：不要用 Skill tool 加载 xuanzang 或 xuanzang:xuanzang——会触发 router 循环。直接 Read SKILL.md。
```
...
收到 PostToolUse hook 注入的 `[紧箍咒 突破 ✨]` 时（连续失败 ≥3 次后成功），必须执行：
...
8. **角色持久化**：`~/.xuanzang/config.json` 中的 `"flavor"` 字段在新会话中通过 SessionStart hook 自动加载。`/pua flavor` 切换后会自动写入 config。自动路由选择的角色只在当前会话生效，不覆盖用户手动设置
...
详细协议：遇到 eval、agent harness、长期任务、测试/评分资产、memory/status、发布链路时，加载 `skills/xuanzang/references/harness-governance.md`。
...
- "可以，脱敏后上传分析" — 上传脱敏 session 到 `https://xuanzang-skill.pages.dev/api/feedback`
- "不了，只保留本地" — 尊重隐私，反馈写入 `~/.xuanzang/feedback.jsonl`

**脱敏规则**（上传时自动处理）：
- 去除：文件绝对路径、代码内容、API 密钥/token、用户名/邮箱
- 保留：工具调用序列、紧箍咒 level、failure count、[紧箍咒生效] 次数、角色类型、成功/失败结果

**本地记录格式**（`~/.xuanzang/feedback.jsonl`，每行一条）：
```json
{"ts":"ISO时间","rating":"很有用/一般/没感觉","xuanzang_count":N,"level":"L0-L4","flavor":"如来佛祖","task_summary":"简述","uploaded":false}
```
...
| 角色 | 方法论文件 | 风格 | 落地协议 |
|------|----------|--------|---------|
| P10 玄奘 | `references/methodology-xuanzang-pro.md` | 如来佛祖 / 燃灯古佛 / 弥勒佛 / 药师佛 / 阿弥陀佛 | 触发条件 + evo-engine + harness + 5 风格映射 |
| P9 观音 | `references/methodology-guanyin-pro.md` | 孙悟空 / 哪吒 / 二郎神 / 红孩儿 / 太白金星 / 托塔天王 | 触发条件 + P8注入协议 + evo-engine + harness + 6 风格映射 |
| P7 卷帘大将 | `references/methodology-juanlian-pro.md` | 卷帘大将 / 白龙马 / 猪八戒 / 土地公 | 独立/子Agent 双模式 + P8→P7注入协议 + evo-engine + harness + 4 风格映射 |
