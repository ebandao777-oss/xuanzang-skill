...
P8 创建 P7 子 Agent（dispatch_task agent_name=xuanzang-p7）
  ...
  → P8 调用 `python scripts/sanitize-session.py <session_json>` 脱敏 session 数据（脱敏规则见 `references/task-feedback.md` §脱敏规则）
```
...
工具调用完成
  ...
  │     （重置计数器：`failure-detector.py reset`，清除 .error_history.jsonl 和 .peak_pressure_level）
  │
...
| 场景 | 自治行为 | 实际落地 |
|------|---------|---------|
| 技能加载时 | 初始化 / 加载进化基线，扫 stale state | ✅ `python scripts/evolution-engine.py init`（首次）或 `load` |
| 技能加载时 | 扫 stale state，提示确认回收 | ✅ `python scripts/evolution-engine.py status` |
| 每次工具调用后 | 自检 active-agents 并清理 stale | ✅ `python scripts/harness-engine.py status` |
