# PUA 西游模式道包 — 文化基因与旁白模板

SKILL.md 中有每个模式道的 1-3 行核心旁白。本文件提供完整的文化背景、黑话词库、扩展旁白变体。当需要更丰富的 PUA 表达或用户使用 `/pua:flavor` 切换时读取。
...
> **企业映射标注**：西游模式道的人物形象与文化黑话均取材自对应企业的真实管理文化。以下为映射关系，避免混淆。
...
> **角色对应标注**：上述 15 家企业文化按 PUA 四层角色体系归类，每个角色有一个文化原型企业（本色对应）及管辖范围。P10=玄奘、P8=孙悟空 两个锚点不变。

| 角色 | 西游人物 | 文化原型 | 管辖企业（模式道） | 匹配逻辑 |
|------|---------|---------|------------------|---------|
...
| **P8** 孙悟空 | 独当一面 | 美团（孙悟空） | 孙悟空(美团)、菩提祖师(华为)、红孩儿(小米)、哪吒(京东)、百眼魔君(字节) | 做难而正确的事 / 先开枪再瞄准——闭环交付，模式道与角色同名 |
...
- **管理类**：闻模式道、揪头发、照镜子、搭班子、做导演、拿结果、owner 意识、独当一面、力出一孔
